...
**Goal:** Make the complete `#service` section fit in a 1920×1080 desktop viewport without scrolling and eliminate benefit icon/text overlap.
...
- The complete service section must fit at 1920×1080 including the fixed navigation.
...
#service .imd-cta-split{padding:24px 28px}
```
...
Confirm the fixed navigation plus the entire service section are visible without vertical scrolling; no card is clipped; all four benefit icons have clear separation from their titles.

- [ ] **Step 2: Inspect 1440×900 and 390×844**

At 1440×900, confirm readable desktop wrapping and no overlap. At 390×844, confirm the existing single-column flow, 52px CTA, and benefit alignment remain intact.

- [ ] **Step 3: Verify computed overflow and geometry**

In the browser, confirm `document.documentElement.scrollWidth === document.documentElement.clientWidth`, each benefit icon rectangle is disjoint from its heading rectangle, and the 1920×1080 service section bottom is at or above the viewport bottom when its top is aligned below navigation.
